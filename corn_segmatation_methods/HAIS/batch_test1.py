        ...
        for i, batch in enumerate(dataloader):
            ...
            if (epoch > cfg.prepare_epochs):
                ...
                if cfg.eval:
                    ...
                    gt_file = os.path.join(cfg.data_root, cfg.dataset, cfg.split, 'gt_' + test_scene_name + '.txt')
...
def test_one_leaf_num(leaf_num):
    """
    叶子个数
    :param leaf_num:
    :return:
    ...
    # 更改 filename_suffix
    cfg.filename_suffix = str(leaf_num) + '.pth'
    model = Network(cfg)
...
def main():
    leaf_num_list = [i for i in range(2, 13)]
    results_dict = {}
    ...
    for leaf_num in leaf_num_list:
        print(f"正在测试 {leaf_num} ...")
        one_avgs = test_one_leaf_num(leaf_num)
        results_dict[leaf_num] = one_avgs
        avg = f"{leaf_num},{one_avgs['all_ap']},{one_avgs['all_ap_50%']},{one_avgs['all_ap_25%']}"
        stem = f"{one_avgs['classes']['stem']['ap']},{one_avgs['classes']['stem']['ap50%']},{one_avgs['classes']['stem']['ap25%']}"
        leaf = f"{one_avgs['classes']['leaf']['ap']},{one_avgs['classes']['leaf']['ap50%']},{one_avgs['classes']['leaf']['ap25%']}"
        res_csv += f"{avg},{stem},{leaf}\n"
    # 保存分割结果

    with open('batch_leaf_num_test_results.csv', 'w') as f:
        f.write(res_csv)
