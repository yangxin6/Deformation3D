...
class Dataset:
    def __init__(self, test=False):
        ...
        self.transform = cfg.transform if cfg.__contains__('transform') else False

        self.train_split = getattr(cfg, 'train_split', 'train')

        self.test = test

        self.meta = cfg.meta if cfg.__contains__('meta') else None
        if test:
    ...
    def load(self, filename):
        if self.test and self.test_split == 'test':
            points, rgb = torch.load(filename)
            return points, rgb
        points, rgb, sem_labels, instance_labels = torch.load(filename)
        return points, rgb, sem_labels, instance_labels

    def trainLoader(self):
        if self.train_split == 'trainval':
            train_file_names = sorted(
                glob.glob(os.path.join(self.data_root, self.data_dir, 'train', '*' + self.filename_suffix))
                + glob.glob(os.path.join(self.data_root, self.data_dir, 'val', '*' + self.filename_suffix))
            )
        ...
        self.train_files = [self.load(i) for i in self.train_file_names]
    ...
    def valLoader(self):
        val_file_names = sorted(
            glob.glob(os.path.join(self.data_root, self.data_dir, 'val', '*' + self.filename_suffix)))
        self.val_files = [self.load(i) for i in val_file_names]
    ...
    def testLoader(self):
        if self.meta is None:
            self.test_file_names = sorted(
                glob.glob(os.path.join(self.data_root, self.data_dir, self.test_split, '*' + self.filename_suffix)))
        else:
            self.test_file_names = [os.path.join(self.data_root, self.data_dir, self.test_split, i.rstrip() + ".pth") for i in
                                    open(os.path.join(self.data_root, self.data_dir, self.meta))]

        self.test_files = [self.load(i) for i in self.test_file_names]
    ...
    def getInstanceInfo2(self, xyz, instance_label):
        '''
        :param xyz: (n, 3)
        :param label: (n), int, (0~n, -100) 语义标签
        :param instance_label: (n), int, (0~nInst-1, -100)
        :return: instance_num, dict
        '''
        instance_info = np.ones((xyz.shape[0], 12),
                                dtype=np.float32) * -100.0  # (n, 9), float, (cx, cy, cz, minx, miny, minz, maxx, maxy, maxz)
        instance_pointnum = []  # (nInst), int
        instance_num = int(instance_label.max()) + 1
        for i_ in range(instance_num):
            ...
            instance_info_i[:, 3:6] = min_xyz_i
            instance_info_i[:, 6:9] = max_xyz_i
            instance_info_i[:, 9:12] = max_xyz_i if i_ != 0 else min_xyz_i
            instance_info[inst_idx_i] = instance_info_i
    ...
    def dataAugment2(self, xyz, jitter=False, flip=False, rot=False):
        m = np.eye(3)
        if jitter:
    ...
    def trainMerge(self, id):
        ...
        for i, idx in enumerate(id):
            ...
            inst_num, inst_infos = self.getInstanceInfo2(xyz_middle, instance_label.astype(np.int32))
            inst_info = inst_infos["instance_info"]  # (n, 9), (cx, cy, cz, minx, miny, minz, maxx, maxy, maxz)
            inst_pointnum = inst_infos["instance_pointnum"]  # (nInst), list

            instance_label = instance_label[np.where(instance_label != -100)] + total_inst_num
